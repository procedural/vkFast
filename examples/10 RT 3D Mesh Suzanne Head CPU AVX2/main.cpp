...
int main() {
  ...
  #define window_w 640 // NOTE(Constantine): Hardcoded.
  #define window_h 360 // NOTE(Constantine): Hardcoded.
  const float aspect_ratio = 9.f / 16.f; // NOTE(Constantine): Hardcoded.

  glfwInit();
  glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
  glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
  GLFWwindow * window = glfwCreateWindow(window_w, window_h, "[vkFast] RT 3D Mesh Suzanne Head CPU AVX2", 0, 0);
  void * window_handle = (void *)glfwGetWin32Window(window);

  gpu_handle_context_t ctx = vfContextInit(1, NULL, FF, LL);
  vfWindowFullscreen(ctx, window_handle, "[vkFast] RT 3D Mesh Suzanne Head CPU AVX2", 700, 700, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
