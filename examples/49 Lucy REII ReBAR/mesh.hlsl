...
interpolated main(uint vid: SV_VertexID, uint iid: SV_InstanceID) {
  float4 pos;
  float4 col;
  if (variables.meshIndex == 0) {
    pos = sharedData[0].meshSuzanneHeadVertexPos[vid];
    col = sharedData[0].meshSuzanneHeadVertexCol[vid];
  } else if (variables.meshIndex == 1) {
    int4 triangleThreeVertexIndices = sharedData[0].meshLucyTrianglesThreeVertexIndices[vid / 3];
    int vertexIndex = triangleThreeVertexIndices[vid % 3];
    pos = sharedData[0].meshLucyVertexPos[vertexIndex];
    col = sharedData[0].meshSuzanneHeadVertexCol[vid % 2904]; // Reusing Suzanne Head's colors, lol
  }
