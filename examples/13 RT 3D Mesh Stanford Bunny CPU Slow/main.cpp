...
static void Slow_IntersectTriangles_MollerTrumboreTest(Ray & ray, const bvhvec4 * verts, uint64_t triangles_count, float camera_pos_x, float camera_pos_y, float camera_pos_z) {
  for (uint64_t triIdx = 0; triIdx < triangles_count; triIdx += 1) {
...
int main() {
  ...
  #define window_w 160 // NOTE(Constantine): Hardcoded.
  #define window_h 90  // NOTE(Constantine): Hardcoded.
  const float aspect_ratio = 9.f / 16.f; // NOTE(Constantine): Hardcoded.

  glfwInit();
  glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
  glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
  GLFWwindow * window = glfwCreateWindow(window_w, window_h, "[vkFast] RT 3D Mesh Stanford Bunny CPU Slow", 0, 0);
#if defined(_WIN32)
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] RT 3D Mesh Stanford Bunny CPU Slow", 700, 700, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
    ...
    for (int y = 0; y < window_h; y += 1)
    {
      ...
      for (int x = 0; x < window_w; x += 1)
      {
        ...
        for (int s = 0; s < msaaSamplesCount; s += 1) {
          ...
          Slow_IntersectTriangles_MollerTrumboreTest(ray, vertex, vertex_count / 3, camera_pos[0], camera_pos[1], camera_pos[2]);
          rays[s] = ray;
        }
