...
int main() {
  ...
  GLFWwindow * window = glfwCreateWindow(window_w, window_h, "[vkFast] REII Texture From Disk", 0, 0);
#if defined(_WIN32)
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] REII Texture From Disk", window_w, window_h, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  mesh_state.programPipelineInfo.variables_slot        = 3;
  ...
  RedHandleSampler sampler = reiiCreateSampler(ctx, NULL, REII_SAMPLER_FILTERING_LINEAR, REII_SAMPLER_FILTERING_LINEAR, REII_SAMPLER_BEHAVIOR_OUTSIDE_TEXTURE_COORDINATE_REPEAT, REII_SAMPLER_BEHAVIOR_OUTSIDE_TEXTURE_COORDINATE_REPEAT, 1);
  ...
  REDGPU_2_EXPECTFL(image_file_fd != NULL || !"You need to have a copy of '08 RT Triangle Textured CPU/texture.ppm' file, converted to .png format with an alpha channel, in this example's folder.");
  int image_w = 0;
  int image_h = 0;
  ...
  memcpy(texture_upload_scratch_buffer.cpu_scratch_buffer_ptr, image, image_w * image_h * image_channels_count);
