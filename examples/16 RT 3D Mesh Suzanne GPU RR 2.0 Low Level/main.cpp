...
int main() {
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] RT 3D Mesh Suzanne GPU RR 2.0 Low Level", 700, 700, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    RadeonRays::IntersectionApiImpl * mapi = (RadeonRays::IntersectionApiImpl *)api;
    //mapi->m_device->QueryIntersection(rays_buffer, rays_vector.size(), hits_buffer, NULL, NULL);
    {
      uint64_t numrays = rays_vector.size();
      auto ray_buffer = static_cast<RadeonRays::CalcBufferHolder const*>(rays_buffer)->m_buffer.get();
      auto hit_buffer = static_cast<RadeonRays::CalcBufferHolder const*>(hits_buffer)->m_buffer.get();

      //mapi->m_device->m_intersector->QueryIntersection(0, ray_buffer, numrays, hit_buffer, NULL, NULL);
      {
        const int kWorkGroupSize = 64;
        int queueidx = 0;

        auto& func = mapi->m_device->m_intersector->m_gpudata->isect_func;

        // Set args
        int arg = 0;
        int offset = 0;

        func->SetArg(arg++, mapi->m_device->m_intersector->m_gpudata->bvh);
        func->SetArg(arg++, mapi->m_device->m_intersector->m_gpudata->vertices);
        func->SetArg(arg++, mapi->m_device->m_intersector->m_gpudata->faces);
        func->SetArg(arg++, mapi->m_device->m_intersector->m_gpudata->shapes);
        func->SetArg(arg++, ray_buffer);
        func->SetArg(arg++, sizeof(offset), &offset);
        func->SetArg(arg++, sizeof(numrays), &numrays);
        func->SetArg(arg++, hit_buffer);

        size_t localsize = kWorkGroupSize;
        size_t globalsize = ((numrays + kWorkGroupSize - 1) / kWorkGroupSize) * kWorkGroupSize;

        //mapi->m_device->m_intersector->m_device->Execute(func, queueidx, globalsize, localsize, NULL);
        {
          auto m_anvil_device = mapi->m_device->m_intersector->m_device->m_anvil_device;

          Calc::Function* vulkan_function = Calc::ConstCast<Calc::Function>( func );
          ...
          if ( nullptr == new_descriptor_set )
          {
            ...
            new_descriptor_set = new Anvil::DescriptorSetGroup( m_anvil_device, false, 1U );

            // add bindings and items (Buffers) to the new* DSG
            for ( uint32_t i = 0; i < number_of_parameters; ++i )
            {
              const Calc::Buffer* parameter = vulkan_function->GetParameters()[ i ];
              Calc::Buffer* buffer = Calc::ConstCast<Calc::Buffer>( parameter );
          ...
          mapi->m_device->m_intersector->m_device->StartRecording();

          // attach pipeline
          mapi->m_device->m_intersector->m_device->m_command_buffer->record_bind_pipeline( VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_id );
          ...
          mapi->m_device->m_intersector->m_device->m_command_buffer->record_bind_descriptor_sets(
          ...
          for ( uint32_t i = 0; i < number_of_parameters; ++i )
          {
            ...
            Anvil::BufferBarrier bufferBarrier( VK_ACCESS_HOST_WRITE_BIT,
                                                ...
                                                mapi->m_device->m_intersector->m_device->GetQueue()->get_queue_family_index(),
                                                mapi->m_device->m_intersector->m_device->GetQueue()->get_queue_family_index(),
                                                buffer->GetAnvilBuffer(),
                                                0,
                                                buffer->GetSize() );

            mapi->m_device->m_intersector->m_device->m_command_buffer->record_pipeline_barrier( VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                                                    VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                                                    VK_FALSE,
                                                    0, nullptr,
                                                    1, &bufferBarrier);
            // tell buffer that we are used by this submit
            buffer->SetFenceId( mapi->m_device->m_intersector->m_device->GetFenceId() );
          }
