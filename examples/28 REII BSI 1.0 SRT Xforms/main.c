...
//\\rc rawbuild debug ` -g -O0`
//\\rc rawbuild release,release-fast ` -O2`
//\\rc rawbuild ` main.c blender.c ../../vkfast.c C:/RedGpuSDK/redgpu.c C:/RedGpuSDK/redgpu_2.c C:/RedGpuSDK/redgpu_32.c ../../extra/Banzai/vkfast_extra_banzai.c ../../extra/Banzai/vkfast_extra_banzai_pointer.c "../../extra/CPU GPU Array/vkfast_extra_cpu_gpu_array.c" ../../extra/REII/vkfast_extra_reii.c ../Common/glfw-3.4.bin.WIN64/lib-mingw-w64/libglfw3.a -lgdi32`
//\\rc rawbuild end

//\\rc rawbuild begin mingw-clang-termux-64-bit
//\\rc rawbuild `x86_64-w64-mingw32-clang -DVKFAST_INCLUDE_TERMUX_PATHS main.c blender.c ../../vkfast.c /data/data/com.termux/files/home/RedGpuSDK/redgpu.c /data/data/com.termux/files/home/RedGpuSDK/redgpu_2.c /data/data/com.termux/files/home/RedGpuSDK/redgpu_32.c ../../extra/Banzai/vkfast_extra_banzai.c ../../extra/Banzai/vkfast_extra_banzai_pointer.c "../../extra/CPU GPU Array/vkfast_extra_cpu_gpu_array.c" ../../extra/REII/vkfast_extra_reii.c ../Common/glfw-3.4.bin.WIN64/lib-mingw-w64/libglfw3.a -luser32 -lshell32 -lgdi32`
//\\rc rawbuild end
...
int main() {
  ...
  GLFWwindow * window = glfwCreateWindow(window_w, window_h, "[vkFast] REII BSI 1.0 SRT Xforms", 0, 0);
#if defined(_WIN32)
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] REII BSI 1.0 SRT Xforms", window_w, window_h, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    // Draw Blender scene
    {
      for (int j = 0; j < BLENDER_MESH_ENUMS_COUNT; j += 1) {
        for (int i = blender_submesh_tri_begin[j]; i < blender_submesh_tri_end[j]; i += 1) {
          int v0i = blender_mesh_indices[i].x;
          int v1i = blender_mesh_indices[i].y;
          int v2i = blender_mesh_indices[i].z;
          
          struct float4 v0 = blender_mesh_vertices[v0i];
          struct float4 v1 = blender_mesh_vertices[v1i];
          struct float4 v2 = blender_mesh_vertices[v2i];

          // Scale

          v0.x *= blender_submesh_xform_scale[j].x;
          v0.y *= blender_submesh_xform_scale[j].y;
          v0.z *= blender_submesh_xform_scale[j].z;

          v1.x *= blender_submesh_xform_scale[j].x;
          v1.y *= blender_submesh_xform_scale[j].y;
          v1.z *= blender_submesh_xform_scale[j].z;

          v2.x *= blender_submesh_xform_scale[j].x;
          v2.y *= blender_submesh_xform_scale[j].y;
          v2.z *= blender_submesh_xform_scale[j].z;

          // Rotate

          quatRotateVec3Fast(&v0.x, &v0.x, &blender_submesh_xform_rotation_quaternion[j].x);
          quatRotateVec3Fast(&v1.x, &v1.x, &blender_submesh_xform_rotation_quaternion[j].x);
          quatRotateVec3Fast(&v2.x, &v2.x, &blender_submesh_xform_rotation_quaternion[j].x);

          // Translate

          vec3Add(&v0.x, &blender_submesh_xform_translation[j].x, &v0.x);
          vec3Add(&v1.x, &blender_submesh_xform_translation[j].x, &v1.x);
          vec3Add(&v2.x, &blender_submesh_xform_translation[j].x, &v2.x);
