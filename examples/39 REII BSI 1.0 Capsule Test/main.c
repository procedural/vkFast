...
int main() {
  ...
  GLFWwindow * window = glfwCreateWindow(window_w, window_h, "[vkFast] REII BSI 1.0 Capsule Test", 0, 0);
#if defined(_WIN32)
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] REII BSI 1.0 Capsule Test", window_w, window_h, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  RedStructDeclarationMember slots_capsule[2] = {0};
  ...
  mesh_state_compile_info_capsule.variables_slot              = 2;
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    bindings_info.max_storage_binds_count     = 4;
    ...
    // Draw Blender scene
    {
      for (int j = 0; j < BLENDER_MESH_ENUMS_COUNT; j += 1) {
        for (int i = mesh_capsule_submesh_tri_begin[j]; i < mesh_capsule_submesh_tri_end[j]; i += 1) {
          ...
          v0.x *= mesh_capsule_submesh_xform_scale[j].x;
          v0.y *= mesh_capsule_submesh_xform_scale[j].y;
          v0.z *= mesh_capsule_submesh_xform_scale[j].z;

          v1.x *= mesh_capsule_submesh_xform_scale[j].x;
          v1.y *= mesh_capsule_submesh_xform_scale[j].y;
          v1.z *= mesh_capsule_submesh_xform_scale[j].z;

          v2.x *= mesh_capsule_submesh_xform_scale[j].x;
          v2.y *= mesh_capsule_submesh_xform_scale[j].y;
          v2.z *= mesh_capsule_submesh_xform_scale[j].z;

          // Rotate

          quatRotateVec3Fast(&v0.x, &v0.x, &mesh_capsule_submesh_xform_rotation_quaternion[j].x);
          quatRotateVec3Fast(&v1.x, &v1.x, &mesh_capsule_submesh_xform_rotation_quaternion[j].x);
          quatRotateVec3Fast(&v2.x, &v2.x, &mesh_capsule_submesh_xform_rotation_quaternion[j].x);

          // Translate

          vec3Add(&v0.x, &mesh_capsule_submesh_xform_translation[j].x, &v0.x);
          vec3Add(&v1.x, &mesh_capsule_submesh_xform_translation[j].x, &v1.x);
          vec3Add(&v2.x, &mesh_capsule_submesh_xform_translation[j].x, &v2.x);
          ...
          reiiCommandMeshColor(ctx, list, v0i * 0.00075f, 0, 0.1f, 1);
          reiiCommandMeshPosition(ctx, list, v0.x, v0.y, v0.z, 1);
          reiiCommandMeshColor(ctx, list, v1i * 0.00075f, 0, 0.1f, 1);
          reiiCommandMeshPosition(ctx, list, v1.x, v1.y, v1.z, 1);
          reiiCommandMeshColor(ctx, list, v2i * 0.00075f, 0, 0.1f, 1);
          reiiCommandMeshPosition(ctx, list, v2.x, v2.y, v2.z, 1);
        }
      }
    }
    reiiCommandMeshEndWithTale64BytesAlign(ctx, list, outputdstex, outputmstex, outputmstex->texture);
