...
int main() {
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] REII Present Stutter Catcher", window_w, window_h, 0, FF, LL);
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    float disabled_camera_rotation_speed = 0.025;
    if (0 && camera_is_enabled == 0) {
      if (mouse_right_mouse_button_state == GLFW_PRESS && mouse_right_mouse_button_state != mouse_right_mouse_button_state_prev) {
        camera_pos.x = (instanceCountX * 3.f) / 2.f;
        camera_pos.y = (instanceCountY * 3.f) / 2.f;
        camera_pos.z = -25.f;
      ...
      float q[4] = {0, 0, 0, 1};
      float axis_y[3] = {0, 1, 0};
      quatFromAxisAngle(q, axis_y, disabled_camera_rotation_speed);
      quatRotateVec3Fast(&camera_pos.x, &camera_pos.x, q);

      float tx = camera_pos.x - ((instanceCountX * 3.f) / 2.f);
      float ty = camera_pos.y - ((instanceCountY * 3.f) / 2.f);
      float tz = camera_pos.z - ((instanceCountZ * 3.f) / 2.f);

      float vectorLength = sqrtf(tx*tx + ty*ty + tz*tz);
      if (vectorLength > 0.f) {
        tx /= vectorLength;
        ty /= vectorLength;
        tz /= vectorLength;
      ...
      // Quaternion from look at vector
      {
        float up_x = 0;
        float up_y = 1;
        float up_z = 0;
        ...
        // Right = up x forward (and normalize)
        float right_x = 0;
        float right_y = 0;
        float right_z = 0;
        ...
        float l_r = sqrtf(right_x*right_x + right_y*right_y + right_z*right_z);
        if (l_r > 0.f) {
          right_x /= l_r;
          right_y /= l_r;
          right_z /= l_r;
        ...
        float actual_up_y = 0;
        float actual_up_z = 0;
        actual_up_x = forward_y * right_z - forward_z * right_y;
        actual_up_y = forward_z * right_x - forward_x * right_z;
        actual_up_z = forward_x * right_y - forward_y * right_x;

        // Convert Matrix to Quaternion
        float q[4] = {0, 0, 0, 1};
        float trace = right_x + actual_up_y + forward_z;
        if (trace > 0.0f) {
          float s = 0.5f / sqrtf(trace + 1.0f);
          q[0] = (actual_up_z - forward_y  ) * s;
          q[1] = (forward_x   - right_z    ) * s;
          q[2] = (right_y     - actual_up_x) * s;
          q[3] = 0.25f / s;
        } else {
          if (right_x > actual_up_y && right_x > forward_z) {
            float s = 2.0f * sqrtf(1.0f + right_x - actual_up_y - forward_z);
            q[0] = 0.25f * s;
            q[1] = (right_y     + actual_up_x) / s;
            q[2] = (right_z     + forward_x  ) / s;
            q[3] = (actual_up_z - forward_y  ) / s;
          } else if (actual_up_y > forward_z) {
            float s = 2.0f * sqrtf(1.0f + actual_up_y - right_x - forward_z);
            q[0] = (right_y     + actual_up_x) / s;
            q[1] = 0.25f * s;
            q[2] = (actual_up_z + forward_y  ) / s;
            q[3] = (forward_x   - right_z    ) / s;
          } else {
            float s = 2.0f * sqrtf(1.0f + forward_z - right_x - actual_up_y);
            q[0] = (right_z     + forward_x  ) / s;
            q[1] = (actual_up_z + forward_y  ) / s;
            q[2] = 0.25f * s;
            q[3] = (right_y     - actual_up_x) / s;
          }
        }

        {
          float iq[4] = {0, 0, 0, 1};
          float axis[3] = {0, 1, 0};
          quatRotateVec3Fast(axis, axis, q);
          quatFromAxisAngle(iq, axis, 180.f * (/*M_PI*/3.141592653f/180.f));
          quatMul(q, iq, q);
        }
