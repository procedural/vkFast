...
int main() {
  ...
  GLFWwindow * window = glfwCreateWindow(window_w, window_h, "[vkFast] REII Translation", 0, 0);
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] REII Translation", window_w, window_h, 0, FF, LL);
  ...
  mesh_state_compile_info.variables_bytes_count                     = 1 * sizeof(ReiiVec4);
  mesh_state_compile_info.struct_members_count                      = countof(slots);
  mesh_state_compile_info.struct_members                            = slots;
  mesh_state_compile_info.optional_shader_source_file_path_vertex   = NULL;
  mesh_state_compile_info.optional_shader_source_file_path_fragment = NULL;
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    reiiCommandBindVariablesCopy(ctx, list, 0, 1 * sizeof(ReiiVec4), &camera_pos);
