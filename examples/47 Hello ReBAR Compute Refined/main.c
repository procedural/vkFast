...
int main(int ArgsCount, char ** Args) {
  ...
  gpu_program_pipeline_info_t ppiAdd        = {0};
  RedStructDeclarationMember  ppiAddSlots[] = {
    {
      /*slot*/  0,
  ...
  ppi                        = &ppiAdd;
  ppi->struct_members_count  = countof(ppiAddSlots);
  ppi->struct_members        = ppiAddSlots;
  ...
  uint64_t pp = vfProgramPipelineCreateCompute(ctx, cs, &ppiAdd, NULL, FF, LL);
...

  {
    ...
    vfBatchBindNewBindingsSet(ctx, batch, ppiAdd.struct_members_count, ppiAdd.struct_members, FF, LL);
