...
int main() {
  ...
  vfctx2.doNotDestroyRawContext = 1; // Since we donate raw context from ctx.
  ...
  ctx2allocsizes.bytes_count_for_memory_present_pixels_type_cpu_upload = 0;
  ...
  vfStorageCreate(ctx2, &storage_output_info, &storage_output_cpu, FF, LL);
  storage_output_info.storage_type = GPU_STORAGE_TYPE_GPU_ONLY;
  gpu_storage_t storage_output_gpu = {0};
  vfStorageCreate(ctx2, &storage_output_info, &storage_output_gpu, FF, LL);
  ...
  vfContextDeinit(ctx2, FF, LL); // NOTE(Constantine): Derived from ctx custom contexts must be deinited first.
