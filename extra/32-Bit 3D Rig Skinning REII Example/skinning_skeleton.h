...
#include "skinning_math.h"		// GET THE TYPE FOR MATH STRUCTURES
...
typedef struct {
	float	*__ptr32 data;			/* LIST OF DATA POINTS */
...
typedef struct {
	...
	long		polyCnt;
	tObjFrame	*__ptr32 frame[MAX_FRAMES];		/* LIST OF OBJECT FRAMES */
...
struct t_Bone
{
	...
	// HIERARCHY INFO
	t_Bone	*__ptr32 parent;					// POINTER TO PARENT BONE
	int 	childCnt;					// COUNT OF CHILD BONES
	t_Bone	*__ptr32 children;					// POINTER TO CHILDREN
	// TRANSFORMATION INFO
	...
	float	*__ptr32 primChannel;				// POINTER TO PRIMARY CHANNEL OF ANIMATION
	...
	float	*__ptr32 secChannel;				// POINTER TO SECONDARY CHANNEL OF ANIMATION
	...
	tObjDesc	*__ptr32 desc;			/* POINTER TO DESCRIPTIONS */
	long		cur_desc;		/* NUMBER OF MIP MODELS */
	int		visualCnt;					// COUNT OF ATTACHED VISUAL ELEMENTS
	long	*__ptr32 visuals;					// POINTER TO VISUALS
	int		*__ptr32 CV_ptr;					// POINTER TO CONTROL VERTICES
	float	*__ptr32 CV_weight;					// POINTER TO ARRAY OF WEIGHTING VALUES
	// COLLISION ELEMENTS
