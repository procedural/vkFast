...
#ifdef VKFAST_EXTRA_SYCL_EMBREE_CPU
  #define gpu_cpu_malloc_shared(X, Y) malloc(X)
  #define gpu_cpu_free(X, Y) free(X)
#else
  #define gpu_cpu_malloc_shared sycl::malloc_shared
  #define gpu_cpu_free sycl::free
#endif
...
int main() {
    ...
    float* vertices = (float*)gpu_cpu_malloc_shared(3 * 3 * sizeof(float), queue);
    unsigned int* indices = (unsigned int*)gpu_cpu_malloc_shared(3 * sizeof(unsigned int), queue);
    ...
    uint8_t* pixels = (uint8_t*)gpu_cpu_malloc_shared(WIDTH * HEIGHT * 3, queue);
    ...
    #ifndef VKFAST_EXTRA_SYCL_EMBREE_CPU
    queue.wait();
    #endif
    ...
    // 7. Cleanup
    gpu_cpu_free(pixels, queue);
    gpu_cpu_free(vertices, queue);
    gpu_cpu_free(indices, queue);
