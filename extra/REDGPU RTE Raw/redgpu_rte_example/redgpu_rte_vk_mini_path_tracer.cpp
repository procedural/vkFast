...
#include "../redgpu_rte/nvpro_core/third_party/stb/stb_image_write.h"
#define TINYOBJLOADER_IMPLEMENTATION
#include "../redgpu_rte/nvpro_core/third_party/tinyobjloader/tiny_obj_loader.h"

#include "../redgpu_rte/nvpro_core/nvh/fileoperations.hpp"         // For nvh::loadFile
#include "../redgpu_rte/nvpro_core/nvvk/context_vk.hpp"
#include "../redgpu_rte/nvpro_core/nvvk/descriptorsets_vk.hpp"     // For nvvk::DescriptorSetContainer
#include "../redgpu_rte/nvpro_core/nvvk/error_vk.hpp"
#include "../redgpu_rte/nvpro_core/nvvk/raytraceKHR_vk.hpp"        // For nvvk::RaytracingBuilderKHR
#include "../redgpu_rte/nvpro_core/nvvk/resourceallocator_vk.hpp"  // For NVVK memory allocators
#include "../redgpu_rte/nvpro_core/nvvk/shaders_vk.hpp"            // For nvvk::createShaderModule
#include "../redgpu_rte/nvpro_core/nvvk/structs_vk.hpp"            // For nvvk::make
...
#include "redgpu_rte_source_code.cpp"

int main(int argc, const char** argv)
{
  const unsigned nvvkInstanceExtensionsCount = 2;
  const char * nvvkInstanceExtensions[nvvkInstanceExtensionsCount] = {
    "VK_KHR_surface",
    "VK_KHR_win32_surface",
  };
  const unsigned nvvkDeviceExtensionsCount = 3;
  ...
  rteInfo.vkInstanceLayersCount          = 0;
  rteInfo.vkInstanceLayers               = 0;
  ...
  rteInfo.debugSeverityFilterMask        = VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
  rteInfo.requestQueueFamilysCount       = 0;
  rteInfo.requestQueueFamilysFlags       = 0;
  rteInfo.requestQueueFamilysQueuesCount = 0;
  rteInfo.requestQueueFamilysPriority    = 0;
  rteInfo.nvvkContext                    = 0;
  RedContext redcontext = 0;
  redRteCreateContext(malloc, free, 0, 0, 0, RED_SDK_VERSION_1_0_135, 0, 0, 0, 0, 0, 0, 0, &redcontext, 0, __FILE__, __LINE__, 0, &rteInfo);
  ...
  const std::string        exePath = "../redgpu_rte/"; // (argv[0], std::string(argv[0]).find_last_of("/\\") + 1);
  std::vector<std::string> searchPaths = {"../redgpu_rte/vk_mini_path_tracer/"}; // {exePath + PROJECT_RELDIRECTORY, exePath + PROJECT_RELDIRECTORY "..",
  ...
  VkShaderModule rayTraceModule =
      nvvk::createShaderModule(context, nvh::loadFile("../redgpu_rte/vk_mini_path_tracer/vk_mini_path_tracer/shaders/raytrace.comp.glsl.spv", true, searchPaths));
