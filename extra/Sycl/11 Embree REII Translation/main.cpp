...
int main() {
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] Embree REII Translation", WIDTH, HEIGHT, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  RTCGeometry worldGeom = rtcNewGeometry(rtm_rtc_device, RTC_GEOMETRY_TYPE_INSTANCE);
  rtcSetGeometryInstancedScene(worldGeom, suzanneInstanceScene);
  //...
  rtcSetGeometryTransform(worldGeom, 0, RTC_FORMAT_FLOAT3X4_COLUMN_MAJOR, m3x4);
  rtcCommitGeometry(worldGeom);

  RTCScene worldInstanceScene = rtcNewScene(rtm_rtc_device);
  unsigned worldInstanceId = rtcAttachGeometry(worldInstanceScene, worldGeom);
  rtcCommitScene(worldInstanceScene);
  ...
  RTCTraversable traversable = rtcGetSceneTraversable(worldInstanceScene);
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    {
      camera_pos.x += glfwGetKey(window, GLFW_KEY_D) * 0.025f;
      camera_pos.x -= glfwGetKey(window, GLFW_KEY_A) * 0.025f;

      camera_pos.y += glfwGetKey(window, GLFW_KEY_E) * 0.025f;
      camera_pos.y -= glfwGetKey(window, GLFW_KEY_Q) * 0.025f;
    ...
    {
      // Update World's instance transform separately here
      RTCGeometry geom = rtcGetGeometry(worldInstanceScene, worldInstanceId);
      // Column 3 (Translation vector)
      m3x4[9]  = -camera_pos.x;
