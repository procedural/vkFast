...
int main() {
  ...
  GLFWwindow * window = glfwCreateWindow(WIDTH, HEIGHT, "[vkFast] Embree REII Lucy", 0, 0);
  #if defined(_WIN32)
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] Embree REII Lucy", WIDTH, HEIGHT, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  RTCGeometry lucyMeshGeom = rtcNewGeometry(rtm_rtc_device, RTC_GEOMETRY_TYPE_TRIANGLE);
  rtcSetSharedGeometryBuffer(lucyMeshGeom, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3, &objVertices[0], 0, sizeof(float3), 14027872);
  rtcSetSharedGeometryBuffer(lucyMeshGeom, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3, &objIndices[0], 0, sizeof(uint3), 28055728);
  rtcCommitGeometry(lucyMeshGeom);
  ...
  RTCScene lucyMeshScene = rtcNewScene(rtm_rtc_device);
  rtcAttachGeometry(lucyMeshScene, lucyMeshGeom);
  rtcCommitScene(lucyMeshScene);
  ...
  // Create an instance of Lucy mesh
  RTCGeometry lucyInstanceGeom = rtcNewGeometry(rtm_rtc_device, RTC_GEOMETRY_TYPE_INSTANCE);
  rtcSetGeometryInstancedScene(lucyInstanceGeom, lucyMeshScene);
  rtcSetGeometryTransform(lucyInstanceGeom, 0, RTC_FORMAT_FLOAT3X4_COLUMN_MAJOR, m3x4);
  rtcCommitGeometry(lucyInstanceGeom);

  RTCScene allInstancesScene = rtcNewScene(rtm_rtc_device);
  unsigned suzanneInstanceId = rtcAttachGeometry(allInstancesScene, suzanneInstanceGeom);
  unsigned lucyInstanceId    = rtcAttachGeometry(allInstancesScene, lucyInstanceGeom);
  rtcCommitScene(allInstancesScene);
  ...
  rtcSetGeometryInstancedScene(worldXform1Geom, allInstancesScene);
  ...
  {
    RTCGeometry geom = rtcGetGeometry(allInstancesScene, suzanneInstanceId);
    // Column 3 (Translation vector)
    m3x4[9]  = 0.0f;
    m3x4[10] = 0.0f;
    m3x4[11] = 0.0f;
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    const float camera_move_speed      = 0.9f;
