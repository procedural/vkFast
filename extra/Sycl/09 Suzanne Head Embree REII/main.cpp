...
int main() {
  ...
  vfWindowFullscreen(ctx, NULL, "[vkFast] Suzanne Head Embree REII", WIDTH, HEIGHT, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  for (int i = 0, counter = 0; i < 2904; i += 1) {
    ...
    vertices[i].z = suzanne_head_mesh_vertices[i * 3 + 2] * scale + 2; // NOTE(Constantine): For now, hardcode camera or mesh vertex offset in Z axis.
  }
