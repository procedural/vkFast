...
Ray RayInit(glm::vec3 origin, glm::vec3 direction, double tmax = 1e300, unsigned rayMask = RAY_MASK_INTERSECT_ALL) {
  Ray ray = {};

  ray.O = origin, ray.D = direction;
  double rl = 1.0 / sqrt( ray.D.x * ray.D.x + ray.D.y * ray.D.y + ray.D.z * ray.D.z );
  ray.D.x *= rl, ray.D.y *= rl, ray.D.z *= rl;
  ray.rD.x = 1.0 / ray.D.x, ray.rD.y = 1.0 / ray.D.y, ray.rD.z = 1.0 / ray.D.z;
  ray.hit.u = ray.hit.v = 0, ray.hit.t = tmax;
  ray.instIdx = 0;
  ray.mask = rayMask & RAY_MASK_INTERSECT_ALL;

  return ray;
}

static bool IntersectBoxByBboxIndex(glm::vec4 pos, glm::vec4 invDir, float * bvh, uint64_t bboxIdx, float t) {
  glm::vec4 bbox_min;
  glm::vec4 bbox_max;
  ...
  glm::vec4 t0 = (bbox_min - pos) * invDir;
  glm::vec4 t1 = (bbox_max - pos) * invDir;
...
static void IntersectTriangleByTriangleIndex_MollerTrumboreTest(Ray & ray, const glm::vec4 * verts, uint64_t triIdx, float camera_pos_x, float camera_pos_y, float camera_pos_z) {
  const uint64_t i0 = triIdx * 3 + 0;
  const uint64_t i1 = triIdx * 3 + 1;
  const uint64_t i2 = triIdx * 3 + 2;

  glm::vec4 vert0 = verts[i0];
  glm::vec4 vert1 = verts[i1];
  glm::vec4 vert2 = verts[i2];

  vert0 = vert0 - glm::vec4(camera_pos_x, camera_pos_y, camera_pos_z, 0);
  vert1 = vert1 - glm::vec4(camera_pos_x, camera_pos_y, camera_pos_z, 0);
  vert2 = vert2 - glm::vec4(camera_pos_x, camera_pos_y, camera_pos_z, 0);

  const glm::vec4 v0_ = vert0;
  const glm::vec3 v0 = v0_;
  const glm::vec3 e1 = vert1 - v0_;
  const glm::vec3 e2 = vert2 - v0_;
  const float tmax = ray.hit.t;

  const glm::vec3 h = glm::cross( ray.D, e2 );
  const float a = glm::dot( e1, h );
  if (fabs( a ) < 0.000001f) return;
  const float f = 1 / a;
  const glm::vec3 s = ray.O - v0;
...
static void BVH_IntersectTriangles_MollerTrumboreTest(Ray & ray, const glm::vec4 * verts, uint64_t triangles_count, float * bvh, float camera_pos_x, float camera_pos_y, float camera_pos_z) {
  glm::vec4 ray_o = glm::vec4(ray.O.x, ray.O.y, ray.O.z, 0);
...
int main() {
  #define WINDOW_WIDTH  720 // NOTE(Constantine): Hardcoded.
  #define WINDOW_HEIGHT 405 // NOTE(Constantine): Hardcoded.
  const float aspect_ratio = 9.f / 16.f; // NOTE(Constantine): Hardcoded.

  glfwInit();
  glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
  glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
  GLFWwindow * window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "[vkFast] Stanford Bunny GPU Slow BVH Part 3", 0, 0);
#if defined(_WIN32)
  ...
  vfWindowFullscreen(ctx, window_handle, "[vkFast] Stanford Bunny GPU Slow BVH Part 3", WINDOW_WIDTH, WINDOW_HEIGHT, 0, RED_PRESENT_VSYNC_MODE_ON, FF, LL);
  ...
  for (uint64_t i = 0, bbox_min_or_max_count = (sizeof(bunny_bvh_stack) / sizeof(float)) / 3; i < bbox_min_or_max_count; i += 1) {
    float new_scale = 0.025f;
    bunny_bvh_stack[i * 3 + 0] *= new_scale;
    bunny_bvh_stack[i * 3 + 1] *= new_scale;
    bunny_bvh_stack[i * 3 + 2] *= new_scale;
  }

  float * bunny_bvh = (float *)sycl::malloc_shared(sizeof(bunny_bvh_stack), sycl_queue);

  for (uint64_t i = 0, bbox_min_or_max_count = (sizeof(bunny_bvh_stack) / sizeof(float)) / 3; i < bbox_min_or_max_count; i += 1) {
    bunny_bvh[i * 3 + 0] = bunny_bvh_stack[i * 3 + 0];
    bunny_bvh[i * 3 + 1] = bunny_bvh_stack[i * 3 + 1];
    bunny_bvh[i * 3 + 2] = bunny_bvh_stack[i * 3 + 2];
  }

  const uint64_t vertex_count = (sizeof(bunny) / sizeof(float)) / 3;

  glm::vec4 * vertex = (glm::vec4 *)sycl::malloc_shared(sizeof(glm::vec4) * vertex_count, sycl_queue);
  ...
  glm::vec3 camera_axis_x = glm::vec3(1, 0, 0);
  glm::vec3 camera_axis_y = glm::vec3(0, 1, 0);
  glm::vec3 camera_axis_z = glm::vec3(0, 0, 1);
  ...
  while (glfwWindowShouldClose(window) == 0) {
    ...
    glm::vec3 up = camera_axis_y * aspect_ratio;
    glm::vec3 C  = 2.f * camera_axis_z;
    glm::vec3 p1 = C - camera_axis_x + up;
    glm::vec3 p2 = C + camera_axis_x + up;
    glm::vec3 p3 = C - camera_axis_x - up;
    ...
    sycl_queue.submit([&](sycl::handler& sycl_cgh) {
      sycl_cgh.parallel_for(sycl::range<2>(WINDOW_HEIGHT, WINDOW_WIDTH), [=](sycl::id<2> sycl_id) {
        ...
        for (int s = 0; s < msaaSamplesCount; s += 1) {
          float u = (float)(x + 0.5f + msaa_samples_offset_table_x[s]) / (float)(WINDOW_WIDTH);
          float v = (float)(y + 0.5f + msaa_samples_offset_table_y[s]) / (float)(WINDOW_HEIGHT);
          glm::vec3 P = p1 + u * (p2 - p1) + v * (p3 - p1);
          glm::vec3 P_normalized = glm::normalize(P);
          Ray ray = RayInit(glm::vec3(0, 0, 0), P_normalized);
        ...
        pixels->pixels[y][x][0] = color_average;
        pixels->pixels[y][x][1] = color_average;
        pixels->pixels[y][x][2] = color_average;
        pixels->pixels[y][x][3] = 255;
      });
    });
  ...
  // Cleanup
  sycl::free(vertex, sycl_queue);
  sycl::free(bunny_bvh, sycl_queue);
